...
namespace Models
{
    public class WorkOrderWorks_DS : I_DataServer
    {
        ...
        private const string szxWORK_ORDER_WORKS_TableKey = "WorkOrderWorks";

        public WorkOrderWorks_DS(DatabaseConnection dbcv) 
        {
            dbcx = dbcv;
            fdsx = new Fields();
            ...
            dbcx.DBTables.AddDBTable(szxWORK_ORDER_WORKS_TableKey, 
                                     ref dbtx);

            XX_GetFieldsCollectionOverTableName(ref fdsx);

            dbcx.CreateTableForFirstTimeUseIf(szxWORK_ORDER_WORKS_TableKey, 
                                              fdsx);

        }

        public void GetWorkOrderWorkOverWorkId(long lvWorkId,
                                               ref Fields fdsr,
...

        {
            ...
            szSQL = "SELECT * FROM WorkOrderWorks WHERE WorkId =" + Convert.ToString(lvWorkId);
...

        {
            ...
            szSQL = "SELECT * FROM WorkOrderWorks "
                  + " WHERE WorkOrderNumber = '" + fdsr.ItemIsField("WorkOrderNumber").StringValue + "' "
...

        {
            ...
            szSQL = "SELECT * FROM WorkOrderWorks "
                  ...
                  + " AND   WorkOrderNumber = '" + fdsr.ItemIsField("WorkOrderNumber").StringValue + "' "
                  + " ORDER BY WorkId ASC LIMIT 1";
        ...
        public void RemoveFromDataBaseOverWorkOrder(Fields fdsv,
                                                           ref bool brErrorFound,
                                                           ref string szrErrorMessage)
        {
                                        string szSQL = string.Empty;

            szSQL = "DELETE FROM WorkOrderWorks "
                  + " WHERE   "
                  + " WorkId = " + fdsv.ItemIsField("WorkId").StringValue
                  + " AND WorkOrderNumber = '" + fdsv.ItemIsField("WorkOrderNumber").StringValue + "' ";
                                       ...
                                       ref string szrErrorMessage)
        {
            ...
            szSQL = "DELETE FROM WorkOrderWorks WHERE WorkId = " + fdsv.ItemIsField("WorkId").StringValue;
        ...
        private void XX_GetNextWorkId(ref long lrWorkId)
        {
            ...
            szSQL = "SELECT MAX(WorkId) FROM WorkOrderWorks";
        ...
        private void XX_GetFieldsCollectionOverTableName(ref Fields fdsr)
        {
            ...
            szTableFilePath = dbcx.ApplicationPathCarrier.TablesPath + @"\" + "WorkOrderWorks.tbl";
            ...
            while (true)
            {
                ...
                if (!bFileExists)
                {
                    MessageBox.Show("WorkOrderWorks table not found",
                                    "Missing Table File");
                }
